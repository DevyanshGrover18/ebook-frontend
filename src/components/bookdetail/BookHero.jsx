...
function BookHero({ book }) {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/cart/${cartId}/items`,
        {
          method: "POST",
      ...
      showToast(
        `${book.title} added to cart! (local state fallback)`,
        "success",
      );
    }
  };
  ...
  const handlePrev = () =>
    setActiveImageIndex((p) => (p === 0 ? allImages.length - 1 : p - 1));
  const handleNext = () =>
    setActiveImageIndex((p) => (p === allImages.length - 1 ? 0 : p + 1));
          ...
          <div className="flex flex-col items-center lg:items-start gap-4 w-full">
            ...
            <div className="flex items-center gap-3 text-on-surface-variant">
              <button
                type="button"
                aria-label="Share"
                className="flex cursor-pointer items-center gap-2 text-label-md font-label-md hover:text-secondary transition-colors"
              >
          ...
          <div className="flex flex-col gap-6">
            ...
            <div>
              ...
              <p className="font-body-lg text-body-lg text-on-surface-variant">
                By{" "}
                <span className="text-secondary font-semibold">{author}</span>
              </p>
            </div>
            ...
            <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-xl p-4 sm:p-6 flex flex-col gap-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <Button
                  ...
                  size="lg"
                  rounded="lg"
                  className="flex-1 gap-3 cursor-pointer !bg-surface-container !text-on-surface !border-outline-variant hover:!bg-surface-container-high"
              ...
              <div className="flex flex-wrap gap-3 sm:gap-4 pt-2 border-t border-outline-variant/20">
                <div className="flex items-center gap-1.5 text-label-sm text-on-surface-variant">
                  <Shield className="w-4 h-4 text-secondary" /> 14-day refund
                  guarantee
                </div>
                <div className="flex items-center gap-1.5 text-label-sm text-on-surface-variant">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {[
                ...
                <div key={label} className="flex flex-col gap-1">
                  <dt className="text-label-sm text-on-surface-variant uppercase tracking-wider">
                    {label}
                  </dt>
